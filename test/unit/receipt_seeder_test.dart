  ...
  group('ReceiptSeeder Unit Tests', () {
    ...
    test('generate24Receipts generates 2 records per month for 12 months', () {
      ...
      for (int i = 0; i < 12; i++) {
        ...
        expect(countInMonth, equals(2), reason: 'Month ${targetYearMonth.month}/${targetYearMonth.year} should have 2 receipts');
      }
    });

    test('All generated receipts satisfy item count, pricing, qty, and emoji-free constraints', () {
      final receipts = ReceiptSeeder.generate24Receipts();
      ...
      for (final r in receipts) {
        ...
        for (final item in r.lineItems) {
          ...
          expect(item.totalPrice, equals(((item.unitPrice! * item.quantity!) * 100).round() / 100.0));
        }

        // Total amount matches sum of line items
        final sumItems = r.lineItems.fold<double>(0.0, (acc, item) => acc + item.totalPrice!);
        expect(r.amount, equals((sumItems * 100).round() / 100.0));

        // Category constraints: max 3 category tags, emoji-free
        final tags = r.category.split(',').map((c) => c.trim()).where((c) => c.isNotEmpty).toList();
        expect(tags.length, greaterThanOrEqualTo(1));
        expect(tags.length, lessThanOrEqualTo(3));
        expect(emojiRegExp.hasMatch(r.category), isFalse, reason: 'Category should contain no emojis: ${r.category}');
